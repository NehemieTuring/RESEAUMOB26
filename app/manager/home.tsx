...
import { roleLabelKey } from '../../src/api/roles';
...
export default function HomeScreen() {
    ...
    const quickActions = isPlatformAdmin
        ? [
            { id: 'newFleet', icon: 'copy-outline', label: t('home.newFleet'), route: '/manager/fleets' },
            ...(canManageManagers ? [{ id: 'managers', icon: 'people-outline', label: t('home.managers'), route: '/manager/managers' }] : []),
            { id: 'reports', icon: 'bar-chart-outline', label: t('home.viewReports'), route: '/manager/reports' },
            { id: 'orgProfile', icon: 'business-outline', label: t('moreMenu.orgProfile'), route: '/organization-profile' },
        ]
        : [
            ...(canCreateVehicles ? [{ id: 'addVehicle', icon: 'car-outline', label: t('home.addVehicle'), route: '/manager/vehicles' }] : []),
            ...(canCreateDrivers ? [{ id: 'addDriver', icon: 'person-add-outline', label: t('home.addDriver'), route: '/manager/drivers' }] : []),
            ...(canOperateTrips ? [{ id: 'newTrip', icon: 'navigate-outline', label: t('home.newTrip'), route: '/manager/map' }] : []),
            { id: 'reports', icon: 'bar-chart-outline', label: t('home.viewReports'), route: '/manager/reports' },
        ];
                                ...
                                if (type === 'vehicle') {
                                    return (
                                        <TouchableOpacity
                                            ...
                                            onPress={() => router.push({ pathname: '/manager/vehicles', params: { search: data.vehicleRegistrationNumber, registration: data.vehicleRegistrationNumber } } as any)}
                                        >
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                            onPress={() => router.push({ pathname: '/manager/drivers', params: { search: data.driverEmail, email: data.driverEmail } } as any)}
                                        >
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                            onPress={() => router.push({ pathname: '/manager/fleets', params: { search: data.fleetName, name: data.fleetName } } as any)}
                                        >
                    ...
                    <>
                        ...
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <TouchableOpacity onPress={() => router.push('/manager/vehicles')}>
                            ...
                            {recentVehicles.length > 0 ? (
                                recentVehicles.map((vehicle, index) => (
                                    <TouchableOpacity
                                        ...
                                        style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                        onPress={() => router.push({ pathname: '/manager/vehicles', params: { search: vehicle.vehicleRegistrationNumber, registration: vehicle.vehicleRegistrationNumber } } as any)}
                                    >
                                        <View style={[styles.vehicleIcon, { backgroundColor: colors.primaryBlue + '20' }]}>
                                            <Ionicons name="car" size={22} color={colors.primaryBlue} />
                                        </View>
