...
import { DashboardHeader, ConfirmModal, BellIcon } from '../../src/components';
...
export default function MoreScreen() {
    ...
    const { canManageManagers, canOperateTrips, isManager } = useFleetRole();
    ...
    const menuSections = [
        {
            ...
            items: [
                canManageManagers ? { id: 'managers', title: t('navigation.managers'), icon: 'people', route: '/manager/managers', color: colors.accentPurple || '#8b5cf6' } : null,
                { id: 'trips', title: canOperateTrips ? 'Trajets' : 'Trajets (supervision)', icon: 'map', route: '/manager/trips', color: colors.primaryCyan || '#06b6d4' },
                { id: 'incidents', title: t('navigation.incidents'), icon: 'warning', route: '/manager/incidents', color: colors.warningText },
                { id: 'zones', title: t('navigation.zones'), icon: 'location', route: '/manager/zones', color: colors.successText },
            ].filter((item): item is any => item !== null),
        },
        {
            title: t('more.analytics'),
            items: [
                notificationsEnabled ? { id: 'notifications', title: t('notifications.title'), icon: 'notifications', route: '/manager/notifications', color: colors.warningText } : null,
                { id: 'history', title: t('navigation.history'), icon: 'time', route: '/manager/history', color: colors.primaryBlue },
                { id: 'reports', title: t('navigation.reports'), icon: 'bar-chart', route: '/manager/reports', color: colors.primaryCyan || '#22d3ee' },
            ].filter((item): item is any => item !== null),
        },
        {
            ...
            items: [
                ...
                isManager ? { id: 'org_profile', title: t('moreMenu.orgProfile'), icon: 'business', route: '/organization-profile', color: colors.primaryCyan } : null,
                { id: 'subscription', title: t('navigation.subscription'), icon: 'card', route: '/manager/subscription', color: colors.accentGold || '#f59e0b' },
                { id: 'support', title: t('navigation.support'), icon: 'help-circle', route: '/manager/support', color: colors.infoText },
                { id: 'settings', title: t('navigation.settings'), icon: 'settings', route: '/manager/settings', color: colors.textSecondary },
                                ...
                                >
                                    <View style={[styles.iconContainer, { backgroundColor: item.isDestructive ? colors.errorBg + '30' : item.color + '20' }]}>
                                        {item.id === 'notifications' ? (
                                            <BellIcon size={22} color={item.color} />
                                        ) : (
