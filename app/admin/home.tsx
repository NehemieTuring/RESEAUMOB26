...
import { vehicleApi, driverApi, fleetApi, notificationApi, orgResourcesApi } from '../../src/services';
...
import { isPlatformAdmin as roleIsPlatformAdmin, resolveFleetRole, roleLabelKey } from '../../src/api/roles';
...
export default function HomeScreen() {
    ...
    const fetchDashboardData = async (user?: any) => {
        try {
            ...
            const adminView = roleIsPlatformAdmin(resolveFleetRole(user)) || isPlatformAdmin;
            const notificationFetch = notificationApi.getAll();

            if (adminView) {
                const [summaryRes, vehiclesRes, fleetsRes, driversRes, recentRes, notifsRes] =
                    await Promise.allSettled([
                        orgResourcesApi.getSummary(),
                        orgResourcesApi.getVehicles(),
                        orgResourcesApi.getFleets(),
                        orgResourcesApi.getDrivers(),
                        orgResourcesApi.getRecentVehicles(3),
                        notificationFetch,
                    ]);
                ...
                setStats({
                    totalVehicles: summary?.vehicleCount ?? vehiclesData.length,
                    activeVehicles: summary?.availableVehicleCount ?? 0,
                    totalDrivers: summary?.driverCount ?? driversData.length,
                    alertsToday: unreadNotifications.length,
                    totalFleets: summary?.fleetCount ?? fleetsData.length,
                });

                setAllVehicles(vehiclesData);
                setAllDrivers(driversData);
                setAllFleets(fleetsData);
                setRecentVehicles(recentData.length ? recentData : vehiclesData.slice(0, 3));
    ...
    const quickActions = isPlatformAdmin
        ? [
            { id: 'newFleet', icon: 'copy-outline', label: t('home.newFleet'), route: '/admin/fleets' },
            ...(canManageManagers ? [{ id: 'managers', icon: 'people-outline', label: t('home.managers'), route: '/admin/managers' }] : []),
            { id: 'reports', icon: 'bar-chart-outline', label: t('home.viewReports'), route: '/admin/reports' },
            { id: 'orgProfile', icon: 'business-outline', label: t('moreMenu.orgProfile'), route: '/organization-profile' },
        ]
        : [
            ...(canCreateVehicles ? [{ id: 'addVehicle', icon: 'car-outline', label: t('home.addVehicle'), route: '/admin/vehicles' }] : []),
            ...(canCreateDrivers ? [{ id: 'addDriver', icon: 'person-add-outline', label: t('home.addDriver'), route: '/admin/drivers' }] : []),
            ...(canOperateTrips ? [{ id: 'newTrip', icon: 'navigate-outline', label: t('home.newTrip'), route: '/admin/map' }] : []),
            { id: 'reports', icon: 'bar-chart-outline', label: t('home.viewReports'), route: '/admin/reports' },
        ];
    ...
    const statsData = [
        {
            id: 'vehicles',
            icon: 'car',
            title: isPlatformAdmin ? t('stats.platformVehicles') : t('stats.myVehicles'),
        ...
        },
        {
            id: 'drivers',
            icon: 'people',
            title: isPlatformAdmin ? t('stats.platformDrivers') : t('stats.myDrivers'),
        ...
        },
        {
            id: 'fleets',
            icon: 'grid',
            title: isPlatformAdmin ? t('stats.allFleets') : t('stats.myFleets'),
            value: stats.totalFleets,
            color: colors.accentGold || '#f59e0b',
        },
    ];

    const homeSubtitle = isSuperAdmin
        ? t('home.subtitleSuperAdmin')
        : isPlatformAdmin
            ? t('home.subtitleAdmin')
            : t('home.subtitleManager');
                                ...
                                if (type === 'vehicle') {
                                    return (
                                        <TouchableOpacity
                                            ...
                                            onPress={() => router.push({ pathname: '/admin/vehicles', params: { search: data.vehicleRegistrationNumber, registration: data.vehicleRegistrationNumber } } as any)}
                                        >
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                            onPress={() => router.push({ pathname: '/admin/drivers', params: { search: data.driverEmail, email: data.driverEmail } } as any)}
                                        >
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                            onPress={() => router.push({ pathname: '/admin/fleets', params: { search: data.fleetName, name: data.fleetName } } as any)}
                                        >
                    ...
                    <>
                        ...
                        <View style={styles.headerTitleContainer}>
                            <View style={styles.welcomeContainer}>
                                ...
                                <Text style={[styles.roleHint, { color: colors.textMuted }]}>{homeSubtitle}</Text>
                            </View>
                            {role ? (
                                <View style={[styles.roleBadge, { backgroundColor: colors.primaryBlue + '20', borderColor: colors.primaryBlue }]}>
                                    <Text style={[styles.roleBadgeText, { color: colors.primaryBlue }]}>{t(roleLabelKey(role))}</Text>
                                </View>
                            ) : null}
                        </View>
                        ...
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>
                                    {isPlatformAdmin ? t('home.recentVehiclesPlatform') : t('home.recentVehicles')}
                                </Text>
                                <TouchableOpacity onPress={() => router.push('/admin/vehicles')}>
                            ...
                            {recentVehicles.length > 0 ? (
                                recentVehicles.map((vehicle, index) => (
                                    <TouchableOpacity
                                        ...
                                        style={[styles.vehicleCard, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}
                                        onPress={() => router.push({ pathname: '/admin/vehicles', params: { search: vehicle.vehicleRegistrationNumber, registration: vehicle.vehicleRegistrationNumber } } as any)}
                                    >
                                        <View style={[styles.vehicleIcon, { backgroundColor: colors.primaryBlue + '20' }]}>
                                            <Ionicons name="car" size={22} color={colors.primaryBlue} />
                                        </View>
                                        <View style={styles.vehicleInfo}>
                                            <Text style={[styles.vehicleName, { color: colors.textPrimary }]}>
                                                {vehicle.vehicleMake} {vehicle.vehicleName || vehicle.vehicleModel}
                                            </Text>
                                            <Text style={[styles.vehicleReg, { color: colors.textSecondary }]}>
...
const styles = StyleSheet.create({
    ...
    greeting: { fontSize: 16, fontWeight: '600', marginTop: 4 },
    title: { fontSize: 24, fontWeight: '700' },
    roleHint: { fontSize: 13, marginTop: 4 },
    roleBadge: {
