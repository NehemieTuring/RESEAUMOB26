...
import { fleetManagerApi, FleetManager, orgResourcesApi } from '../../src/services';
import {
    DashboardHeader,
    ConfirmModal,
    DataDetailsModal,
...
export default function ManagersScreen() {
    ...
    const { ready, role, canManageManagers, isSuperAdmin } = useFleetRole();
    ...
    useEffect(() => {
        if (ready && role && !canManageManagers) {
            router.replace('/admin/home');
        }
    }, [ready, role, canManageManagers, router]);

    const fetchManagers = async (silent = false) => {
        try {
            if (!silent) {
                setError(null);
            ...
            const data = await orgResourcesApi.getManagers();
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            ...
            <ResponsivePageHeader
                ...
                subtitle={isSuperAdmin ? t('managers.subtitleSuperAdmin') : t('managers.subtitleAdmin')}
                actionLabel="Nouveau"
                actionIcon="add"
            ...
            <ConfirmModal
                ...
                message={`Voulez-vous vraiment retirer ${managerToDelete?.managerFirstName} ${managerToDelete?.managerLastName} ? Le compte sera désactivé (suppression logique) et disparaîtra des listes.`}
