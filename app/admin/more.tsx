...
export default function MoreScreen() {
    ...
    const { isPlatformAdmin, canManageManagers, canOperateTrips } = useFleetRole();
    ...
    const menuSections = [
        {
            ...
            items: [
                canManageManagers ? { id: 'managers', title: t('navigation.managers'), icon: 'people', route: '/admin/managers', color: colors.accentPurple || '#8b5cf6' } : null,
                { id: 'trips', title: canOperateTrips ? 'Trajets' : 'Trajets (supervision)', icon: 'map', route: '/admin/trips', color: colors.primaryCyan || '#06b6d4' },
                { id: 'incidents', title: t('navigation.incidents'), icon: 'warning', route: '/admin/incidents', color: colors.warningText },
                { id: 'zones', title: t('navigation.zones'), icon: 'location', route: '/admin/zones', color: colors.successText },
            ].filter((item): item is any => item !== null),
        },
        {
            title: t('more.analytics'),
            items: [
                notificationsEnabled ? { id: 'notifications', title: t('notifications.title'), icon: 'notifications', route: '/admin/notifications', color: colors.warningText } : null,
                { id: 'history', title: t('navigation.history'), icon: 'time', route: '/admin/history', color: colors.primaryBlue },
                { id: 'reports', title: t('navigation.reports'), icon: 'bar-chart', route: '/admin/reports', color: colors.primaryCyan || '#22d3ee' },
            ].filter((item): item is any => item !== null),
        },
        {
            ...
            items: [
                ...
                isPlatformAdmin ? { id: 'org_profile', title: t('moreMenu.orgProfile'), icon: 'business', route: '/organization-profile', color: colors.primaryCyan } : null,
                { id: 'subscription', title: t('navigation.subscription'), icon: 'card', route: '/admin/subscription', color: colors.accentGold || '#f59e0b' },
                { id: 'support', title: t('navigation.support'), icon: 'help-circle', route: '/admin/support', color: colors.infoText },
                { id: 'settings', title: t('navigation.settings'), icon: 'settings', route: '/admin/settings', color: colors.textSecondary },
