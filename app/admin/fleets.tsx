...
import { fleetApi, orgResourcesApi, Fleet } from '../../src/services';
import {
    DashboardHeader,
    ConfirmModal,
    DataDetailsModal,
...
export default function FleetsScreen() {
    ...
    const { isPlatformAdmin, canCreateFleets, canMutateFleets } = useFleetRole();

    const fetchFleets = async (silent = false) => {
        try {
            if (!silent) {
                setError(null);
            ...
            const data = await orgResourcesApi.getFleets();
        ...
        return (
            <ResponsiveDataCard
                ...
                fields={[
                    ...
                    ...(isPlatformAdmin
                        ? [{ label: 'Gestionnaire', value: item.fleetManagerName || '-', icon: 'person-outline' as const }]
                        : []),
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            ...
            >
                ...
                <ResponsivePageHeader
                    title={isPlatformAdmin ? t('fleets.titlePlatform') : t('fleets.title')}
                    subtitle={isPlatformAdmin ? t('fleets.subtitleAdmin') : t('fleets.subtitleManager')}
                    actionLabel={canCreateFleets ? 'Nouvelle' : undefined}
                    actionIcon="add"
                    onActionPress={canCreateFleets ? () => setIsCreateModalVisible(true) : undefined}
                />
                ...
                <ConfirmModal
                    ...
                    message={`Voulez-vous vraiment retirer la flotte "${fleetToDelete?.fleetName}" ? Elle sera désactivée (suppression logique) et disparaîtra des listes.`}
