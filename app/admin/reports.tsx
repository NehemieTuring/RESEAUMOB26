...
export default function ReportsScreen() {
    ...
    const fetchData = async (silent = false) => {
        try {
            ...
            const [kpis, fuelData, maintenanceData, incidentData, profile] = await Promise.all([
                orgResourcesApi.getKpis(),
                orgResourcesApi.getFuelRecharges(),
                orgResourcesApi.getMaintenances(),
                orgResourcesApi.getIncidents(),
                orgResourcesApi.getProfile().catch(() => null),
            ]);

            setBackendOnline(true);
            setBackendError(null);

            const total = Number(kpis.counts?.vehicleCount ?? 0);
            const available = Number(kpis.availableVehicles ?? kpis.counts?.availableVehicleCount ?? 0);
            const onTrip = Number(kpis.onTripVehicles ?? 0);
            const inMaintenance = Number(kpis.maintenanceVehicles ?? 0);
            const inactive = Math.max(0, total - available - onTrip - inMaintenance);

            setStats({
                totalFuelCost: Number(kpis.totalFuelCost ?? 0),
                totalFuelQuantity: Number(kpis.totalFuelLiters ?? 0),
                totalMaintenanceCost: Number(kpis.totalMaintenanceCost ?? 0),
                maintenanceCount: Number(kpis.maintenanceCount ?? maintenanceData.length),
                totalDistance: Number(kpis.totalDistanceKm ?? 0),
                tripCount: Number(kpis.completedTrips ?? kpis.counts?.tripCount ?? 0),
                incidentCount: Number(kpis.counts?.incidentCount ?? incidentData.length),
            ...
            if (profile) {
                setOrgInfo({
                    ...
                    logo: profile.logoUrl || profile.photoUrl || null,
                });
            }
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            ...
            {/* Dashboard Header */}
                        <DashboardHeader showSearch={false} onRefresh={() => fetchData(true)} />
