...
import { driverApi, Driver, Vehicle, Fleet, orgResourcesApi } from '../../src/services';
import {
    DashboardHeader,
    ConfirmModal,
    DataDetailsModal,
...
export default function DriversScreen() {
    ...
    const { isPlatformAdmin, canCreateDrivers } = useFleetRole();
    ...
    const fetchDrivers = async (silent = false) => {
        try {
            ...
            const [driversRes, vehiclesRes, fleetsRes] = await Promise.allSettled([
                orgResourcesApi.getDrivers(),
                orgResourcesApi.getVehicles(),
                orgResourcesApi.getFleets(),
            ]);

            const data = driversRes.status === 'fulfilled' ? driversRes.value || [] : [];
    ...
    const handleShowDetails = async (driver: Driver) => {
        ...
        try {
            const full = await driverApi.getById(driver.driverId);
            setSelectedDriver(full);
        } catch {
            /* la fiche déjà chargée reste affichée */
        }
    };
        ...
        return (
            <ResponsiveDataCard
                ...
                fields={[
                    ...
                    { label: 'Flotte', value: fleets.find((f) => f.fleetId === item.fleetId)?.fleetName || '-', icon: 'grid-outline' },
                ]}
                actions={[
                    ...(canCreateDrivers
                        ? [
                            { icon: 'create-outline', label: 'Modifier', onPress: () => handleEditDriver(item), color: colors.textSecondary },
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            ...
            <ResponsivePageHeader
                title={isPlatformAdmin ? t('drivers.titlePlatform') : t('drivers.title')}
                subtitle={isPlatformAdmin ? t('drivers.subtitleAdmin') : t('drivers.subtitleManager')}
                actionLabel={canCreateDrivers ? 'Nouveau' : undefined}
                actionIcon="add"
                onActionPress={canCreateDrivers ? () => setShowCreateModal(true) : undefined}
            />
            ...
            <ConfirmModal
                ...
                message={`Voulez-vous vraiment retirer ${driverToDelete?.driverFirstName} ${driverToDelete?.driverLastName} ? Le chauffeur sera désactivé (suppression logique) et disparaîtra des listes.`}
            ...
            <DataDetailsModal
                ...
                data={[
                    ...
                    { label: 'Identifiant', value: selectedDriver?.username || selectedDriver?.driverId, icon: 'id-card-outline' },
                    ...
                    { label: 'Statut', value: selectedDriver?.driverStatus === 'INACTIVE' ? 'Inactif' : 'Actif', icon: 'checkmark-circle-outline' },
                    { label: 'N° Permis', value: selectedDriver?.driverLicenseNumber, icon: 'card-outline', fullWidth: true },
                    { label: 'Flotte', value: fleets.find((f) => f.fleetId === selectedDriver?.fleetId)?.fleetName || 'Aucune', icon: 'grid-outline', fullWidth: true },
