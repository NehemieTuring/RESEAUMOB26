...
import { TripService as tripApi, type Trip, tripStatusLabel, isScheduledTrip } from '../../src/api/tripService';
...
export default function TripsScreen() {
    ...
    const getStatusText = (status: string) => tripStatusLabel(status);
                ...
                <View style={styles.entitiesRow}>
                    <View style={styles.entityItem}>
                        ...
                        <Text style={[styles.entityText, { color: colors.textPrimary }]} numberOfLines={1}>
                            {item.driverFullName || 'Conducteur non assigné'}
                        </Text>
                    </View>
                    <View style={styles.entityItem}>
                        <Ionicons name="car" size={14} color={colors.textMuted} />
                        <Text style={[styles.entityText, { color: colors.textPrimary }]} numberOfLines={1}>
                            {item.vehicleRegistration || 'Véhicule non assigné'}
                        </Text>
                    </View>
                </View>
            </View>

            {isScheduledTrip(item.status) && (
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
            <DashboardHeader showSearch={false} onRefresh={loadTrips} />
                ...
                ListEmptyComponent={
                    !isLoading ? (
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={[styles.emptyText, { color: colors.textMuted }]}>Aucun trajet dans l'organisation</Text>
                        </View>
                    ) : null
                }
            />
