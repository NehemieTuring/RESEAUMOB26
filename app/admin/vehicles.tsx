...
import { vehicleApi, Vehicle, fleetApi, Fleet, driverApi, Driver, orgResourcesApi } from '../../src/services';
import {
    DashboardHeader,
    ConfirmModal,
    DataDetailsModal,
...
export default function VehiclesScreen() {
    ...
    const { isPlatformAdmin, canCreateVehicles } = useFleetRole();
    ...
    const fetchVehicles = async (silent = false) => {
        try {
            ...
            const [vehiclesRes, fleetsRes, driversRes] = await Promise.allSettled([
                orgResourcesApi.getVehicles(),
                orgResourcesApi.getFleets(),
                orgResourcesApi.getDrivers(),
            ]);

            const data = vehiclesRes.status === 'fulfilled' ? vehiclesRes.value || [] : [];
    ...
    const renderVehicleCard = ({ item }: { item: Vehicle }) => {
        ...
        return (
            <ResponsiveDataCard
                ...
                actions={[
                    ...(canCreateVehicles && !item.fleetId
                        ? [{ icon: 'link-outline', label: 'Assigner', onPress: () => handleAssignFleet(item), color: colors.accentPurple || '#8b5cf6' }]
                        : []),
                    ...(canCreateVehicles
                        ? [
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            <DashboardHeader showSearch={false} onRefresh={onRefresh} />
            ...
            <ResponsivePageHeader
                title={isPlatformAdmin ? t('vehicles.titlePlatform') : t('vehicles.title')}
                subtitle={isPlatformAdmin ? t('vehicles.subtitleAdmin') : t('vehicles.subtitleManager')}
                actionLabel={canCreateVehicles ? 'Nouveau' : undefined}
                actionIcon="add"
                onActionPress={canCreateVehicles ? () => setShowCreateModal(true) : undefined}
            />
            ...
            <ConfirmModal
                ...
                message={`Voulez-vous vraiment retirer le véhicule ${vehicleToDelete?.vehicleMake} ${vehicleToDelete?.vehicleModel} ? Il sera désactivé (suppression logique) et disparaîtra des listes.`}
            ...
            <DataDetailsModal
                ...
                data={[
                    ...
                    { label: 'Année', value: selectedVehicle?.manufacturingYear, icon: 'calendar-outline' },
                    { label: 'Couleur', value: selectedVehicle?.color, icon: 'color-palette-outline' },
                    ...
                    { label: 'Conducteur assigné', value: (() => {
                        const d = drivers.find(x => x.driverId === selectedVehicle?.currentDriverId)
                            || drivers.find(x => x.assignedVehicleId === selectedVehicle?.vehicleId);
                        return d ? `${d.driverFirstName} ${d.driverLastName}`.trim() : 'Aucun';
                    })(), icon: 'person-outline', fullWidth: true },
                    ...
                    { label: 'Niveau Carburant', value: selectedVehicle?.fuelLevel != null ? `${selectedVehicle.fuelLevel}%` : '-', icon: 'speedometer-outline' },
