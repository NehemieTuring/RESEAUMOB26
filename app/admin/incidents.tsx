...
import { orgResourcesApi, incidentApi, Incident, IncidentStatus } from '../../src/services';
import { FleetMap, VehicleMarker, DashboardHeader, ConfirmModal, BackendOfflineBanner, DataDetailsModal } from '../../src/components';
...
export default function IncidentsScreen() {
    ...
    const fetchIncidents = async (silent = false) => {
        try {
            ...
            const data = await orgResourcesApi.getIncidents();
    ...
    const openIncidentDetails = async (item: Incident) => {
        setSelectedIncidentId(item.id);
        setDetailedIncident(item);
        setIsDetailsModalVisible(true);
        try {
            const full = await incidentApi.getById(item.id) as Incident & {
                location?: { longitude?: number; latitude?: number };
                witness?: { name?: string; contact?: string };
                report?: string;
            ...
            setDetailedIncident({
                ...item,
                ...full,
                longitude: full.longitude || loc?.longitude || item.longitude,
                latitude: full.latitude || loc?.latitude || item.latitude,
                reportedBy: full.reportedBy || item.reportedBy,
            });
        } catch {
            // garder les données déjà affichées
        }
    };
    ...
    const formatCost = (cost?: number) => {
        ...
        return `${Number(cost).toLocaleString('fr-FR')} FCFA`;
    };
    ...
    const renderIncidentCard = ({ item }: { item: Incident }) => (
        <TouchableOpacity
            style={[styles.card, {
                ...
                borderColor: selectedIncidentId === item.id ? colors.primaryBlue : colors.borderGlass,
                borderLeftWidth: 4,
                borderLeftColor: getSeverityColor(item.severity),
                borderWidth: selectedIncidentId === item.id ? 2 : 1,
            }]}
            activeOpacity={0.7}
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
            ...
            {showMap && incidentMarkers.length > 0 && (
                <View style={[styles.mapContainer, { backgroundColor: colors.surfaceCard, borderColor: colors.borderGlass }]}>
                    <FleetMap
                        ...
                        selectedVehicleId={selectedIncidentId as any}
            ...
            <DataDetailsModal
                ...
                title={detailedIncident ? getTypeLabel(detailedIncident.type) : 'Incident'}
                description={detailedIncident?.description}
                icon={detailedIncident ? getTypeIcon(detailedIncident.type) : 'alert-circle'}
                iconColor={detailedIncident ? getSeverityColor(detailedIncident.severity) : undefined}
                data={[
                    { label: 'Type', value: detailedIncident ? getTypeLabel(detailedIncident.type) : '-', icon: 'alert-circle-outline' },
                    { label: 'Gravité', value: detailedIncident ? getSeverityLabel(detailedIncident.severity) : '-', icon: 'warning-outline' },
                    { label: 'Statut', value: detailedIncident ? getStatusLabel(detailedIncident.status) : '-', icon: 'flag-outline' },
                    { label: 'Date', value: formatIncidentDate(detailedIncident?.incidentDateTime || detailedIncident?.createdAt), icon: 'calendar-outline' },
                    { label: 'Coût', value: formatCost(detailedIncident?.cost), icon: 'cash-outline' },
                    { label: 'Véhicule', value: detailedIncident?.vehicleRegistration, icon: 'car-outline', fullWidth: true },
                    { label: 'Chauffeur', value: detailedIncident?.driverFullName, icon: 'person-outline', fullWidth: true },
                    { label: 'Signalé par', value: detailedIncident?.reportedBy, icon: 'megaphone-outline', fullWidth: true },
                    { label: 'Description', value: detailedIncident?.description, icon: 'document-text-outline', fullWidth: true },
                    {
                        label: 'Position',
                        value: detailedIncident?.latitude && detailedIncident?.longitude
                            ? `${detailedIncident.latitude}, ${detailedIncident.longitude}`
                            : '-',
                        icon: 'location-outline',
                        fullWidth: true,
                    },
                    { label: 'Rapport', value: (detailedIncident as any)?.report, icon: 'reader-outline', fullWidth: true },
                    { label: 'Témoin', value: (detailedIncident as any)?.witness?.name || (detailedIncident as any)?.witnessName, icon: 'people-outline' },
                    { label: 'Contact témoin', value: (detailedIncident as any)?.witness?.contact || (detailedIncident as any)?.witnessContact, icon: 'call-outline' },
                    { label: 'PV police', value: (detailedIncident as any)?.policeReportNumber, icon: 'shield-outline' },
                    { label: 'Dossier assurance', value: (detailedIncident as any)?.insuranceClaimNumber, icon: 'briefcase-outline' },
                ]}
            />
