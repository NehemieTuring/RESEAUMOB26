...
import { VehicleService, resolveDriverWorkspace } from '../../src/api';
...
export default function DriverVehicleScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      let mappedVehicle = workspace.vehicle;
      if (mappedVehicle && workspace.vehicleId) {
        try {
          const opData = await VehicleService.getOperational(workspace.vehicleId);
          if (opData) {
            if (opData.fuelLevel) mappedVehicle.fuelLevel = parseInt(opData.fuelLevel, 10);
            if (opData.currentSpeed || opData.speed) mappedVehicle.speed = parseInt(String(opData.currentSpeed || opData.speed), 10);
          }
        } catch {
          /* aperçu déjà chargé */
        }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.primaryDark }]} edges={['top']}>
      ...
      <DashboardHeader showSearch={false} onRefresh={loadData} />
