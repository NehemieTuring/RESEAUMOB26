...
import { VehicleService, resolveDriverWorkspace, type Vehicle } from '../../src/api';
...
export default function DriverIncidentsScreen() {
  ...
  const loadData = async () => {
      try {
        ...
        if (workspace.driverId) setDriverId(workspace.driverId);
        if (workspace.vehicle) {
          setVehicles([workspace.vehicle]);
          setSelectedVehicleId(workspace.vehicle.vehicleId);
          return;
        }
        const vehicleList = await VehicleService.getAll().catch(() => []);
