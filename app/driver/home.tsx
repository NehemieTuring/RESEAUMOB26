...
import { DriverService, VehicleService, resolveDriverWorkspace } from '../../src/api';
...
export default function DriverHomeScreen() {
  ...
  const loadData = async () => {
    try {
      const workspace = await resolveDriverWorkspace();
      const userStr = await AsyncStorage.getItem('user');
      setUser(userStr ? JSON.parse(userStr) : null);
      setVehicle(workspace.vehicle);
      setActiveTrip(workspace.activeTrip);
      setFuelLevel(workspace.vehicle?.fuelLevel ?? 0);

      if (workspace.vehicleId) {
        try {
          const opData = await VehicleService.getOperational(workspace.vehicleId);
          if (opData?.fuelLevel) {
            setFuelLevel(parseInt(opData.fuelLevel, 10) || 0);
          }
        } catch {
          /* carburant déjà pris sur le véhicule */
        }
      }

      if (workspace.driverId) {
        try {
          const positionsData = await apiClient.get<any[]>(`/v1/positions/driver/${workspace.driverId}?latest=true`);
          setLastPosition(positionsData && positionsData.length > 0 ? positionsData[0] : null);
        } catch {
          setLastPosition(null);
        }
        try {
          const alarmStatus = await DriverService.getAlarmStatus(workspace.driverId);
          setInAlarm(Boolean(alarmStatus?.inAlarm));
        } catch {
          /* ignore */
        }
      }
        ...
        <Card style={styles.summaryCard}>
          ...
          <View style={styles.summaryItem}>
            ...
            <View style={styles.summaryTexts}>
              ...
              <Text style={[styles.summaryValue, { color: colors.textPrimary }]}>
                {vehicle
                  ? `${vehicle.vehicleMake || vehicle.brand} ${vehicle.vehicleModel || vehicle.model} - ${vehicle.vehicleRegistrationNumber || vehicle.licensePlate}`
                  : 'Aucun véhicule'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.summaryItem}>
            ...
            <View style={styles.summaryTexts}>
              ...
              <Text style={[styles.summaryValue, { color: colors.textPrimary }]}>
                {activeTrip ? `${activeTrip.reference || activeTrip.tripReference || 'Trajet'} - En cours` : 'Aucun trajet actif'}
              </Text>
            </View>
          </View>
